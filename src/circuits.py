...
def main():
    ...
    landmarks = {13: "attn build (contrib d'+1.16)", 15: "TRUTH PEAK",
                 16: "FFN value flip", 17: "erosion band", 18: "erosion band",
                 19: "evaporation ->"}
    ...
    if cross is None:
        print("  no sustained OV>QK crossover found: the static relay does NOT show up")
        print("  in this model's circuit norms. The link to our causal anatomy is NOT")
        print("  supported by weights alone. Honest negative; the erosion story stands")
        print("  on its own causal evidence regardless.")
    else:
        dist = min(abs(cross - 15), abs(cross - 16))
        print(f"  sustained OV/QK > 1 from layer {cross}  "
              f"(distance from truth-peak/flip landmarks: {dist})")
        if dist <= 2:
            print("  -> crossover sits ON the measured boundary: the static routing->writing")
            print("     relay and the causal truth anatomy (peak 15, flip 16) coincide.")
            print("     Three independent views of one phase transition. Strong lead --")
            print("     next step is the same table on a second model + the truth probe there.")
        else:
            print("  -> crossover exists but sits FAR from the truth boundary: the scaling")
            print("     law may be real yet unrelated to where truth lives. Weak/no link.")
